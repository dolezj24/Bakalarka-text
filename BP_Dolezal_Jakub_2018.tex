...
\begin{figure}
	\includegraphics[height=\textwidth, angle=90]{domain_model.eps}
...
\begin{figure}
	\includegraphics[width=\textwidth]{order_activity.eps}
...
\begin{figure}
	\includegraphics[width=\textwidth]{order_cycle.eps}
...
\begin{figure}
	\includegraphics[width=\textwidth]{UI_components.eps}
...
\begin{figure}
	\includegraphics[width=\textwidth]{server_side.eps}
...
\begin{figure}
	\includegraphics[width=\textwidth]{server_side_presenters.eps}
...
\begin{figure}
	\includegraphics[width=\textwidth]{api_presenters.eps}
...
\begin{figure}
	\includegraphics[width=\textwidth]{expedition_plan.eps}
...
\begin{figure}
	\includegraphics[width=\textwidth]{invoice_form.eps}
	\caption{Formulář pro vytvoření faktury}\label{faktura}
\end{figure}

\subsection{Tabulka}
	Obrázek \ref{tabulka} zobrazuje navrženou tabulku pro zobrazení hodnot z~databáze. Základem tabulky je jqxDataTable\cite{jqx_dataTable} od jQWidgets. JqxDataTable poskytuje již hotové stránkování, textové řazení podle jednotlivých sloupců, textové pole pro filtrování a možnost zobrazit vloženou tabulku. Díky rozsáhlému API umožňuje JqxDataTable vytvoření vlastního způsobu vybírání řádek. Komponenty od jQWidgets mají mnoho vlastních stylů. V~návrhové ukázce je použit styl \uv{Light}.
...
\begin{figure}
	\includegraphics[width=\textwidth]{Deployment_view.eps}
...
\subsection{Změna modelu v~nástroji Skipper}
	Úpravu modelu provedeme pomocí formulářů v~nástroji Skipper. Pro nové entity je vhodné nastavit jméno vygenerovaného souboru s~definicí zastřešující třídy. Tvar jména bude typicky \uv{<jméno entity>.php}. Do nové entity je také vhodné vložit atribut primárního klíče nazývaný \uv{id}, ačkoliv ve vygenerovaném souboru entity bude nahrazen použitím identifikačního traitu. Důležité je to především u~entit, kde primární klíč bude sloužit jako cizí klíč v~jiné tabulce. Díky tomu budou správně vygenerovány vazební vlastnosti entit.
...
\begin{figure}
	\dirtree{%
		...
		.3 app\DTcomment{aplikace serverové strany}.
		.4 api.
		.5 presenters\DTcomment{prezentery modulu API}.
		.4 components\DTcomment{Nette formuláře}.
		.4 config.
		.4 factories\DTcomment{továrničky}.
		.4 model\DTcomment{třídy obsluhující akce na databázovým modelem}.
		.4 presenters\DTcomment{prezentery aplikace}.
		.5 templates\DTcomment{vykreslovací šablony Latte}.
		.4 router.
		.4 .htaccess.
		.4 bootstrap.php\DTcomment{spouštěcí soubor Nette}.
		.3 db-mangment-proj\DTcomment{projekt pro manipulaci s~databází}.
		.3 client \DTcomment{aplikace klientské strany}.
		.4 components\DTcomment{komponenty uživatelského rozhraní}.
		.4 index.js\DTcomment{vstupní soubor pro uživatelské komponenty}.
		.4 package.json\DTcomment{závislost komponent}.
		.3 www \DTcomment{viditelné soubory webu}.
		.4 jqwidgets.
		.4 static\DTcomment{minifikovaná verze klientské aplikace}.
		.3 .htaccess\DTcomment{přesměrování do složky www pro Apache}.
		.3 composer.json\DTcomment{závislosti projektu}.
		.2 thesis\DTcomment{zdrojová forma práce ve formátu \LaTeX{}}.
		.3 BP\_Dolezal\_Jakub\_2018.tex.
		.3 mybibliographyfile.bib.
		.1 text\DTcomment{text práce}.
		.2 BP\_Dolezal\_Jakub\_2018.pdf\DTcomment{text práce ve formátu PDF}.
		.2 zadani.pdf\DTcomment{zadání práce ve formátu PDF}.
	}
\end{figure}
