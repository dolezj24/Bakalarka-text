...
\begin{enumerate}
	...
	\begin{itemize}
		\item[FN1] Přehled evidence vydaných faktur a fakturovaných položek a pro jednotlivé faktury:
		\begin{itemize}
...
\begin{enumerate}
	...
	\end{itemize}
	\item[NF5] Editace položek přehledů v modálním okně.
	\item[NF6] V tabulkových přehledech záznamů možnost zobrazit subtabulku pro zobrazení souvisejích položek.
	\item[NF7] Filtrování záznamů v tabulkách nad všemi sloupci.
\end{enumerate}

\subsection{Požadavky na rozhraní}
\begin{enumerate}
	\item[PR1] Integrace do MySQL databáze a souborového systému pro upload příloh.
	\item[PR1] Přívětivé uživatelské rozhraní v ReactJS napomáhající uživateli práci odvést rychle a bezchybně.
\end{enumerate}
...
\section{Výběr nástroje pro modelování tříd}
	Hlavním cílem bylo najít nástroj, který by uměl z namodelovaného schématu vygenerovat třídní model včetně ORM notací pro Doctrine 2, getterů a setterů. Takový nástroj by velice pomohl při vývoji. V první řadě je potřeba pro vizualizaci problémové domény. Za drhuhé poskytne zdrojový kód modelu zastřešující databázové schéma. Vybíral jsem z následujících dvou modelářů.
...
\subsection{Formulář pro naplánování expedice}
	Na obrázku \ref{expedice} je navržený formulář pro naplánování expedice definovaný v funkčním požadavku FN5.
